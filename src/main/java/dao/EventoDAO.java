...
public class EventoDAO {
	...
	public List<Evento> read() {
		...
		try {
			...
			while (rset.next()) {
				Evento evento1 = new Evento(0, 0, sql, sql);

				evento1.setId_Evento(0);
				evento1.setdata_Evento(rset.getString("data_Evento"));
				evento1.setnome(rset.getString("nome"));
				
				((List<Evento>) evento1).add (evento1);
			}} 
			catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public Evento readById(int id_Evento) {
		Evento evento = new Evento(id_Evento, id_Evento, null, null);
